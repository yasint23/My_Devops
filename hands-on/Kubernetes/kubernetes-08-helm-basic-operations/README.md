...
```bash
helm create yasin-chart
```

- See the files of yasin-chart.
...
```bash
rm -rf yasin-chart/templates/*
```

- Create a `configmap.yaml` file under `yasin-chart/templates` folder with following content.

```yaml
apiVersion: v1
kind: ConfigMap
metadata:
  name: yasin-chart-config
data:
  myvalue: "Hello World"
```

- Install the yasin-chart.

```bash
helm install helm-demo yasin-chart
```
...
```bash
kubectl get cm
kubectl describe cm yasin-chart-config
```
...
- Let's create our own values and use it within the template. Update the `yasin-chart/values.yaml` as below.

```yaml
course: DevOps
```

- Edit the yasin-chart/templates/configmap.yaml as below.
...
metadata:
  name: yasin-chart-config
data:
  myvalue: "yasin-chart configmap example"
...
```bash
helm install --debug --dry-run mydryrun yasin-chart
```

- Install the yasin-chart.

```bash
helm install myvalue yasin-chart
```
...
```bash
helm install --debug --dry-run setflag yasin-chart --set course=AWS
```

- Install the yasin-chart.

```bash
helm install setflag yasin-chart --set course=AWS
```
...
```bash
helm install --debug --dry-run builtin-object yasin-chart
```

- Let's try more examples to get more clarity. Update the `yasin-chart/values.yaml` as below.
...
```bash
helm install --debug --dry-run morevalues yasin-chart
```

- So far, we've seen how to place information into a template. But that information is placed into the template unmodified. Sometimes we want to transform the supplied data in a way that makes it more useful to us. Update the `yasin-chart/templates/configmap.yaml` as below.
...
- Update the `yasin-chart/templates/configmap.yaml` as below.
...
- To add installation notes to your chart, simply create a `yasin-chart/NOTES.txt` file. This file is plain text, but it is processed like a template and has all the normal template functions and objects available.

- Let's create a simple `NOTES.txt` file under `yasin-chart/templates` folder.
...
```bash
helm install notes-demo yasin-chart
```
...
```bash
mkdir my-charts
mv yasin-chart-0.1.0.tgz my-charts
...
entries:
  ...
  - apiVersion: v2
    ...
    type: application
    urls:
    - http://<public ip>:8080/yasin-chart-0.1.0.tgz
    version: 0.1.0
...
```bash
cd my-charts
curl --data-binary "@yasin-chart-0.1.0.tgz" http://<public ip>:8080/api/charts
```
...
- Let's see how to maintain the chart version. In `yasin-chart/Chart.yaml`, set the `version` value to `0.1.1`and then package the chart.

```bash
helm package yasin-chart
mv yasin-chart-0.1.1.tgz my-charts
helm repo index my-charts --url http://<public-ip>:8080
```
...
```bash
cd my-charts
curl --data-binary "@yasin-chart-0.1.1.tgz" http://<public ip>:8080/api/charts
```
...
- In yasin-chart/Chart.yaml, set the `version` value to `0.2.0`and push the chart.

```bash
helm cm-push yasin-chart mylocalrepo
```
...
```bash
helm cm-push yasin-chart mylocalrepo --version="1.2.3"
```
...
```bash
helm cm-push yasin-chart mylocalrepo --version="1.2.4"
```
...
helm repo update
helm search repo mylocalrepo
helm install from-local-repo mylocalrepo/yasin-chart
```
...
```bash
helm cm-push yasin-chart mylocalrepo --version="1.2.5"
helm repo update
```
...
```bash
helm upgrade from-local-repo mylocalrepo/yasin-chart
```
...
```bash
helm upgrade from-local-repo mylocalrepo/yasin-chart --version 1.2.4
```
