...
resource "null_resource" "config" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "echo db_server  ansible_host=${aws_instance.managed-node[0].private_ip}  ansible_ssh_private_key_file=~/yasin.pem ansible_user=ec2-user >> inventory.txt",
      "echo web_server  ansible_host=${aws_instance.managed-node[1].private_ip}  ansible_ssh_private_key_file=~/yasin.pem ansible_user=ec2-user >> inventory.txt",
      "chmod 400 yasin.pem"
    ]
  }
