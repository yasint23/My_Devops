...
@app.route("/")
def hello():
    return "<h1>Welcome to devopsyasin web page</h1>"
...
```bash
docker run -d --name welcome -p 8080:80 yasint23/flask     # check webpage from localhost:8080
docker container ls
```
...
```bash
docker build -t yasint23/flask:2.0 -f ./Dockerfile-alpine . 
docker image ls
```
...
```bash
docker run -d --name welcome -p 8080:80 yasint23/flask:2.0
docker ps
```
