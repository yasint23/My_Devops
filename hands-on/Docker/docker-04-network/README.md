...
- Get the IP of `clarus2nd` container. (172.17.0.3)
...
# docker exec -it clarus1st sh (vscode da ctl+p+q yok bu sekilde girelim)
```
...
- Run a `nginx` web server, name the container as `ng`, and bind the web server to host port 8080 command to run alpine shell. Explain `--rm`(run command then remove) and `-p` flags and port binding.
